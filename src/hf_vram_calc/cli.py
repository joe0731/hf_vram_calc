...
def format_memory_size(memory_gib: float) -> str:
    """Format memory size with appropriate unit (in GiB)"""
    if memory_gib >= 1024:
...
def print_parallelization_table(memory_results: list[LlmodelMemoryResult]):
    ...
    # Common GPU memory sizes for reference (in GiB)
    gpu_sizes = [4, 8, 16, 24, 40, 80]
...
def print_detailed_recommendations(memory_results: list[LlmodelMemoryResult]):
    ...
    gpu_table.add_column("Memory\n(GiB)", justify="center", style="cyan", width=10)
    ...
    for gpu_name, gpu_memory, category in displayed_gpus:
        ...
        gpu_table.add_row(
            gpu_name,
            f"[{memory_style}]{gpu_memory:.2f}[/{memory_style}]",
            can_inference,
            can_training,
            can_lora,
        )
