...
class ConfigManager:
    ...
    def get_gpu_types(self) -> List[Tuple[str, float, str]]:
        """
        Get GPU types list with category.

        Returns memory in GiB (converted from nominal GB spec) for accurate memory calculations.
        The gpu_types.json stores nominal memory in GB (e.g., "80GB" for A100),
        but actual usable memory reported by nvidia-smi is in GiB.

        Returns:
            List of tuples: (gpu_name, memory_gib, category)
        """
        from .calculator import convert_gb_to_gib

        return [
            (
                gpu["name"],
                convert_gb_to_gib(gpu["memory_gb"]),
    ...
    def list_gpu_types(self):
        ...
        table.add_column("Nominal\n(GB)", style="green", justify="right")
        table.add_column("Usable\n(GiB)", style="bright_green", justify="right")
        table.add_column("Category", style="yellow")
        table.add_column("Architecture", style="white")

        for gpu in self.gpu_types:
            memory_gib = convert_gb_to_gib(gpu["memory_gb"])
            table.add_row(
                gpu["name"],
                str(gpu["memory_gb"]),
                f"{memory_gib:.2f}",
