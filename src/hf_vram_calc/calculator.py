...
2. Memory Requirements (in GiB):
   - Model Memory: num_parameters × bytes_per_dtype ÷ 1,073,741,824
...
def convert_gb_to_gib(size_gb: float) -> float:
    """
    Convert GB (decimal, 1000^3) to GiB (binary, 1024^3).

    Conversion formula: GiB = GB × (1000^3 / 1024^3) = GB × 0.93132257461548

    Args:
        size_gb: Size in GB (decimal unit, as specified in device specs)

    Returns:
        Size in GiB (binary unit, as used by operating systems)

    Example:
        >>> convert_gb_to_gib(80)  # 80GB A100
        74.50580596923828  # actual usable memory in GiB
    """
...
class VRAMCalculator:
    ...
    def calculate_kv_cache_memory(
        ...
        """
        Calculate KV Cache memory requirements in GiB.
        Formula: KV_Cache (GiB) = 2 × Batch_Size × Sequence_Length × Head_Dim × Num_KV_Heads × Num_Layers × Precision ÷ 1,073,741,824
        ...
        # Convert to GiB (divide by 1,073,741,824 = 1024^3)
        kv_cache_gib = kv_cache_bytes / (1024**3)
        return kv_cache_gib

    def calculate_inference_memory(
        self, model_memory_gib: float, kv_cache_factor: float = None
    ) -> float:
        """Calculate inference memory requirements in GiB (model weights only)"""
        if kv_cache_factor is None:
            kv_cache_factor = self.inference_overhead_factor
        return model_memory_gib * kv_cache_factor
    ...
    def calculate_training_memory(
        ...
        """Calculate training memory requirements in GiB"""
        ...
        return model_memory_gib * optimizer_factor * overhead_factor
    ...
    def calculate_lora_memory(
        ...
        """Calculate LoRA fine-tuning memory requirements in GiB"""
        ...
        total_memory = (
            model_memory_gib + lora_overhead_gib
        ) * self.lora_overhead_factor
        return total_memory
    ...
    def estimate_activation_memory(
        ...
        """Estimate activation memory in GiB"""
        ...
        return total_activation_size / (1024**3)  # convert to GiB

    def calculate_gradient_memory(self, model_memory_gib: float) -> float:
        """Calculate gradient memory requirements in GiB"""
        return model_memory_gib  # gradients have same size as model weights

    def calculate_optimizer_memory(
        self, model_memory_gib: float, optimizer_type: str = "adam"
    ) -> float:
        """Calculate optimizer state memory requirements in GiB"""
        optimizer_factors = {
        ...
        return model_memory_gib * factor
